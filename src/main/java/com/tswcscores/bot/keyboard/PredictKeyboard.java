...
public class PredictKeyboard {
  ...
  public static List<List<Map<String, String>>> homeGoalsKeyboard(long matchId) {
    ...
    for (int g : GOALS_ROW1)
      row1.add(Map.of("text", String.valueOf(g), "callback_data", PICK_AWAY + matchId + ":" + g));
    List<Map<String, String>> row2 = new ArrayList<>();
    for (int g : GOALS_ROW2)
      row2.add(Map.of("text", String.valueOf(g), "callback_data", PICK_AWAY + matchId + ":" + g));
    rows.add(row1);
    rows.add(row2);
    ...
    for (int g : GOALS_ROW1)
      row1.add(
          Map.of(
              ...
              String.valueOf(g),
              "callback_data",
              PICK_CONFIRM + matchId + ":" + homeGoals + ":" + g));
    List<Map<String, String>> row2 = new ArrayList<>();
    for (int g : GOALS_ROW2)
      row2.add(
          Map.of(
  ...
  public static String homePrompt(Match match) {
    return String.format(
        ...
        match.getTitle(), match.getHomeTeam().getDisplayName());
  }

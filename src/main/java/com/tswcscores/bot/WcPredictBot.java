...
public class WcPredictBot {
  ...
  public void handleUpdate(TelegramUpdate update) {
    ...
    switch (command) {
      ...
      default -> {
        ...
        else if (textLower.equals("❓ помощь"))
          telegram.sendMessage(chatId, BotMessageBuilder.help(BOT_VERSION));
  ...
  private void handleMatches(Long chatId, Long telegramId) {
    ...
    if (userOpt.isEmpty()) {
      telegram.sendMessage(chatId, BotMessageBuilder.notRegistered());
      return;
    }

    List<Match> matches =
        matchRepository.findUpcoming(
    ...
    if (matches.isEmpty()) {
      ...
      telegram.sendMessageWithInlineKeyboard(
          chatId,
  ...
  private void handlePredict(Long chatId, Long telegramId, String text) {
    ...
    if (parts.length != 4) {
      telegram.sendMessage(
          chatId,
          "Формат: <code>/predict {id} {гол1} {гол2}</code>\nПример: <code>/predict 42 2 1</code>");
      return;
    }
    try {
      ...
      if (homeScore < 0 || awayScore < 0 || homeScore > 20 || awayScore > 20) {
        telegram.sendMessage(chatId, "❌ Некорректный счёт 😅");
        return;
      }
  ...
  private void handleMyPredictions(Long chatId, Long telegramId) {
    ...
    telegram.sendMessage(
        chatId,
        BotMessageBuilder.myPredictions(
            userOpt.get(), predictionService.getRecentPredictions(userOpt.get())));
  }
  ...
  private void handleCallback(
      com.tswcscores.telegram.TelegramUpdate.TelegramCallbackQuery callback) {
      ...
      matchRepository
          ...
          .ifPresent(
              match -> {
                ...
                if (isGroup) {
                  telegram.sendMessage(
                      dialogChatId, "⚽ Прогноз на <b>" + match.getTitle() + "</b>:");
                }
                telegram.sendMessageWithInlineKeyboard(
                    dialogChatId,
    ...
    if (data.startsWith(PredictKeyboard.PICK_CONFIRM)) {
      ...
      if (userOpt.isEmpty()) {
        if (messageId != null) {
          telegram.editMessage(
              dialogChatId, messageId, BotMessageBuilder.notRegistered(), List.of());
        } else {
      ...
      try {
        var pred = predictionService.savePrediction(userOpt.get(), matchId, homeGoals, awayGoals);
        if (messageId != null) {
          telegram.editMessage(
              dialogChatId, messageId, BotMessageBuilder.predictionSaved(pred), List.of());
        } else {
        ...
        if (messageId != null) {
          telegram.editMessage(dialogChatId, messageId, "⛔ " + e.getMessage(), List.of());
        } else {
  ...
  private void handleSync(Long chatId) {
    ...
    try {
      ...
      telegram.sendMessage(chatId, BotMessageBuilder.syncDone());
    } catch (Exception e) {
      telegram.sendMessage(chatId, "❌ Ошибка: " + e.getMessage());
    }
  }
  ...
  private void handleCalcScore(Long chatId) {
    ...
    try {
      ...
      telegram.sendMessage(chatId, BotMessageBuilder.calcScoreDone());
    } catch (Exception e) {
      telegram.sendMessage(chatId, "❌ Ошибка: " + e.getMessage());
    }
  }

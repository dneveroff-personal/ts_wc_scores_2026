...
public interface PredictionRepository extends JpaRepository<Prediction, Long> {
  ...
  @Query(
      "SELECT p FROM Prediction p JOIN FETCH p.match m WHERE p.user.id = :userId ORDER BY m.utcDate"
          + " DESC")
  ...
  @Query(
      ...
      """)
  List<Prediction> findRecentByUserId(
      @Param("userId") Long userId, @Param("from") java.time.LocalDateTime from);

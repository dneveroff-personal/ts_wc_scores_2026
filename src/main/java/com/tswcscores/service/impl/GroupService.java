    ...
    return chatGroupRepository
        ...
        .orElseGet(
            () -> {
              ...
              return chatGroupRepository.save(
                  ChatGroup.builder().chatId(chatId).title(title).build());
            });
  }
            ...
            group -> {
              boolean exists =
                  userGroupPointsRepository
                      .findByUserIdAndChatGroupId(user.getId(), group.getId())
                      .isPresent();
              if (!exists) {
                log.info("Adding user {} to group {}", user.getDisplayName(), group.getTitle());
                userGroupPointsRepository.save(
                    UserGroupPoints.builder().user(user).chatGroup(group).build());
              }
            });
  }
